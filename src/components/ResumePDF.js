...
// Define standard colors (Hex only)
const colors = {
  blue: "#0F52BA",
  dark: "#171717",
  grey: "#404040",
...
export const ResumePDF = ({ data, isAI }) => {
  ...
  const certs = (isAI && data.aiCertifications) ? data.aiCertifications : (data.certifications || []);
    ...
    <Document title={`${data.name || "Resume"}`}>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.contact}>
          <Text>{data.email} {data.phone ? ` | ${data.phone}` : ""}</Text>
        </View>
          ...
          {skills.length > 0 && (
            <View>
              ...
              <View style={{ flexDirection: "row", flexWrap: "wrap" }}>
                {skills.map((s, i) => (
                  <View key={i} style={{ width: "50%", flexDirection: "row", marginBottom: 2 }}>
                    <Text style={styles.bullet}>•</Text>
